...
mod room;
mod utils;
use rust_raylib::drawing::Draw;
use rust_raylib::{color, KeyboardKey};
...
fn read_input() -> Option<config::Config> {
    ...
    let mut out: config::Config = config::Config {
        cell_size: 25.0,
        scale_size: 6.0,
...
fn display(mut b: building::Building, confg: config::Config) {
    ...
    let mut rl = rust_raylib::Raylib::init_window(
    ...
    while !rl.window_should_close() {
        if rl.is_key_pressed(KeyboardKey::N) {
            floor += 1;
            if floor > b.num_floors() - 1 {
                floor = 0;
            }
        }
        if rl.is_key_pressed(KeyboardKey::P) {
            if floor == 0 {
        ...
        if rl.is_key_pressed(KeyboardKey::W) {
...
fn main() {
    ...
    if confg_opt.is_none() {
        eprintln!(
            "usage: room --floors <how many floors> --rooms <how many rooms on first floor>..."
        );
        exit(1);
    }
    let confg = confg_opt.unwrap();
    let b: building::Building =
        building::generate_building(confg.num_rooms, confg.num_floors, &confg);
